...
export default function CustomFilterDropdown({ x, y, data, onClose, onEdit }) {
    console.log(data, 'we r here')
    const [caseDetail, setCaseDetail] = useState(data);
    const [isEditOpen, setIsEditOpen] = useState(false);

    // Function to handle opening the edit filter
    const handleEditClick = () => {
        setIsEditOpen(true);
        onEdit(caseDetail);
    };

    // Function to close edit filter
    ...
    const handleDelete = async () => {
        try {
            // Send the request to delete the caseDetail
            const response = await fetch('http://localhost:5000/api/ewfm/deleteewfmIDP', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ caseDetail })  // Send the caseDetail to the backend
            });
            ...
            // Show success alert
            alert('Case Detail deleted successfully!');

        } catch (error) {
            console.error('Fetch error:', error);
            alert('An error occurred while deleting the case detail.');
        }
    };
